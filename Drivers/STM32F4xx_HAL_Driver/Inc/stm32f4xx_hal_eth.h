...
#define ETH_RX_DESC_CNT 4U
    ...
    typedef struct
    {
        ...
        FunctionalState UnicastSlowProtocolPacketDetect; /*!< Enable or disables the Detection of Slow Protocol Packets
                                                            with unicast address. */
        ...
        FunctionalState
            DropTCPIPChecksumErrorPacket; /*!< Enables or disables Dropping of TCPIP Checksum Error Packets. */
    ...
    typedef struct
    {
        ...
        uint32_t
            DescriptorSkipLength; /*!< Specifies the number of word to skip between two unchained descriptors (Ring
    ...
    typedef void (*pETH_txPtpCallbackTypeDef)(
        uint32_t* buffer, ETH_TimeStampTypeDef* timestamp); /*!< pointer to an ETH Tx Free function */
/**
 *
 */
...
#define ETH_DMATXDESC_ES \
    ((uint32_t) 0x00008000U) /*!< Error summary: OR of the following bits: UE || ED || EC || LCO || NC || LCA || FF || \
                                JT */
...
#define ETH_DMARXDESC_ES \
    ((uint32_t) 0x00008000U) /*!< Error summary: OR of the following bits: DE || OE || IPC || LC || RWT || RE || CE */
...
#define ETH_DMARXDESC_MAMPCE \
    ((uint32_t) 0x00000001U) /*!< Rx MAC Address/Payload Checksum Error: Rx MAC address matched/ Rx Payload Checksum \
                                Error */
...
#define ETH_DMAPTPRXDESC_IPPT_UDP \
    ((uint32_t) 0x00000001U) /* UDP payload encapsulated in \
                                the IP datagram */
#define ETH_DMAPTPRXDESC_IPPT_TCP \
    ((uint32_t) 0x00000002U) /* TCP payload encapsulated in \
                                the IP datagram */
#define ETH_DMAPTPRXDESC_IPPT_ICMP \
    ((uint32_t) 0x00000003U) /* ICMP payload encapsulated in \
                                the IP datagram */
...
#define ETH_HEADER ((uint32_t) 14U) /*!< 6 byte Dest addr, 6 byte source addr, 2 byte length/type */
